...
### ロジックは人間が見る。バグはエージェントが見つける。

dbt のコードレビューで **人間** が集中すべきは、ビジネスロジックの正しさ、テストケースの妥当性、データコントラクトの合意。**AI エージェント** はバグ検出、規約チェック、未カバー分岐のテスト生成を担当する。

このツールはその橋渡し — テスト定義を可視化して「正しいものがテストされているか？」を、生の YAML/SQL を読まずに素早く判断できるようにする。

### カバレッジ = テスト数ではなく、明示的な判断

100% カバレッジとは「全カラムにテストがある」ことではない。**「全カラムについてテストの有無の判断が明示されている」** こと。

- テスト定義済み → **テスト済**
- `meta: { no_test: true, reason: "..." }` → **除外**（理由付きで意図的に除外）
- 何もなし → **未判断**（レビューで確認すべき）

セキュリティレビューの accepted risk と同じ構造。目標は「テストなしのカラムをゼロにする」ことではなく、「未判断のカラムをゼロにする」こと。
...
- **レビュー結果の自動保存** — 同じブラウザ・同じプロジェクト/ブランチでステータスとコメントを復元
